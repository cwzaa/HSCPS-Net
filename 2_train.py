...
def id_proxy_memory(E2VID, model, loader, device):
    ...
    with torch.no_grad():
        ...
        for (image_grids, labels, cams, indexs) in loader:
            ...
            _, _, embed_feats, _ = model(img_gray)  #L2
...
def train_model(model_ReId, E2VID, optimizer, scheduler, device, num_epochs=50):  #PSA+ISA+HML+ID
    E2VID = E2VID.eval()
    ...
    new_loader = IterLoader(torch.utils.data.DataLoader(datasetnew,
                                             batch_size=opt.batchsize, num_workers=opt.num_workers, drop_last=True,
                                             sampler=ClassUniformlySampler(datasetnew, class_position=3,
                                                                           k=opt.num_instances,
                                                                           acculabels=datasetnew.accumulate_labels),
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            for i in range(200):
                ...
                #img_gray = un_mask_filter(img_gray)
                #img_gray = intensity_rescaler(img_gray)
                img_gray_detach = img_gray.detach()
                [category, feature, _, _] = model_ReId(img_gray_detach)  # feature:embed_feature
                ...
                loss_cl = torch.tensor([0.]).to(device)#cap_memory(feature_l2, labels, acculabels, cams, epoch=epoch, batch_ind=i)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_Bin', default=1, type=int, help='number of channels of spatiotemporal event-voxel')
