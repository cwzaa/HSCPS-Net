...
def split_data(path_, set_):
    ...
    if set_ == "train":
       split_save_dir = "/data/cwz/ReId_without_Id-main/data1/train/"
       step = 1
    else:   
       split_save_dir = "/data/cwz/ReId_without_Id-main/data1/test/"
       step = 5
    ...
    for k in range(0, len(file_array), step):
        ...
        events = np.loadtxt(fname=copy_path, dtype=np.float64,
                   delimiter=' ',
                   usecols=(0, 1, 2, 3))

        if np.size(events, 0) <= 10:#discard the damaged chunks
            # os.remove(copy_path)
            continue
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='/data/cwz/Event_ReId_denoising/out/Event_ReId/')
