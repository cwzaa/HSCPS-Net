...
from reid_utils.CamAwareMemory import CAPMemory, CAPMemorymy2
...
from evaluator import evaluator, evaluatorcam
from reid_utils.sampler import ClassUniformlySampler
from bisect import bisect_right
from torch import nn


def id_proxy_memory(model, loader, device):
    model.eval()
    with torch.no_grad():
...
def train_model(model_ReId, optimizer, scheduler, device, num_epochs=50):  #PSA+ISA+HML+ID
    start_time = time.time()
    writer = SummaryWriter()
    dataloaders_pure, dataset_pure = load_pure_data(opt)

    "construct camera proxy memory bank"
    intra_id_labels, intra_id_features = id_proxy_memory(model_ReId, dataloaders_pure, device)
    cap_memory = CAPMemorymy2(beta=opt.inv_beta, alpha=opt.inv_alpha, all_img_cams=dataset_pure.cam_list, global_labels=dataset_pure.label_list,
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            if epoch >= 1:
                cap_memory.init_memory(model_ReId, dataloaders_pure, device)  #每一轮都初始化memory
            ...
            for i in range(200):
                ...
                """ReId model training"""
                [category, feature, feature_l2, _] = model_ReId(input_voxel)  # feature:embed_feature
            ...
            mapp, map_embed = evaluatorcam(opt, model_ReId, dataloaders=dataloaders_test,
